...
class LoadMutualCountsRequested extends ProfileEvent {
  final String viewerUid;
  final String profileUid;
  LoadMutualCountsRequested({required this.viewerUid, required this.profileUid});
}
...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  String? _viewerUid;
  String? _profileUid;
  String? _friendStatusLoadedFor;
  String? _mutualCountsLoadedFor;
  ...
  ProfileBloc({
    ...
    on<LoadMutualCountsRequested>(_onLoadMutualCounts);
  ...
  Future<void> _onLoadProfile(LoadProfileEvent event, Emitter<ProfileState> emit) async {
    ...
    final visibilityFuture = getRelationshipStatusUseCase(viewerUid: event.viewerUid, profileUid: event.uid);

    if (isOwnProfile && !_ensuredExistsUids.contains(event.uid)) {
      unawaited(ensureProfileExistsUseCase(uid: event.uid, username: event.uid, displayName: event.uid)
          .then((_) => _ensuredExistsUids.add(event.uid))
          .catchError((_) {}));
    }

    _viewerUid = event.viewerUid;
    _profileUid = event.uid;
    _friendStatusLoadedFor = null;
    _mutualCountsLoadedFor = null;
    ...
    final visibilityResult = await visibilityFuture;
    ...
    if (afterVisibility is ProfileLoadedState && !emit.isDone) {
  ...
  void _onProfileLoadTimedOut(_ProfileLoadTimedOut event, Emitter<ProfileState> emit) {
    ...
    if (!_firstSnapshotReceived) {
      emit(ProfileErrorState(message: 'লোড হতে সময় বেশি লাগছে। আবার চেষ্টা করুন।'));
    }
  }
  ...
  Future<void> _onProfileSnapshotReceived(_ProfileSnapshotReceived event, Emitter<ProfileState> emit) async {
    ...
    if (_friendStatusLoadedFor == profileUid) return;
    _friendStatusLoadedFor = profileUid;

    if (visibility == ProfileVisibility.friend) {
      final afterKnownFriend = state;
      if (afterKnownFriend is ProfileLoadedState && !emit.isDone) {
        emit(afterKnownFriend.copyWith(friendRequestStatus: FriendRequestStatus.friends));
      }
      return;
    }
  ...
  Future<void> _onLoadMutualCounts(LoadMutualCountsRequested event, Emitter<ProfileState> emit) async {
    final current = state;
    if (current is! ProfileLoadedState) return;
    if (event.viewerUid == event.profileUid) return;
    if (_mutualCountsLoadedFor == event.profileUid) return;
    _mutualCountsLoadedFor = event.profileUid;

    final mutualResult = await getMutualGroupsCountUseCase(uid: event.viewerUid, otherUid: event.profileUid);
    ...
    final mutualFriendsResult = await getMutualFriendsCountUseCase(uid: event.viewerUid, otherUid: event.profileUid);
  ...
  Future<void> _onUpdateProfile(UpdateProfileRequested event, Emitter<ProfileState> emit) async {
    ...
    try {
      ...
      final refreshed = state;
      if (refreshed is ProfileLoadedState && !emit.isDone) {
        final updatedProfile = refreshed.profile.copyWith(
          displayName: event.displayName,
          username: event.username,
          bio: event.bio,
          location: event.location,
        ...
        emit(refreshed.copyWith(profile: updatedProfile, isSaving: false));
      }
    } catch (error) {
      emit(ProfileErrorState(message: friendlyErrorMessage(error)));
      emit(current.copyWith(isSaving: false));
  ...
  Future<void> _runPhotoUpload({
    ...
    try {
      ...
      final refreshed = state;
      if (refreshed is ProfileLoadedState && !emit.isDone) {
        final updatedProfile = slot == PhotoSlot.avatar
            ? refreshed.profile.copyWith(
        ...
        _currentProfile = updatedProfile;
        emit(_applyUploadStatus(refreshed.copyWith(profile: updatedProfile), slot, const PhotoUploadStatus()));
      }
    } catch (error) {
      final bool isCancelled = slot == PhotoSlot.avatar ? myToken != _avatarUploadToken : myToken != _coverUploadToken;
