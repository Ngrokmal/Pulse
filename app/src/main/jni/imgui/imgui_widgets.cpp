...
bool RoundToggleButton(const char* id, bool* v)
{
    ...
    float speed = 4.0f;           // rotation speed
    if (*v) rot += ImGui::GetIO().DeltaTime * speed;

    // rainbow ring
    int segments = 40;
    for (int i = 0; i < segments; i++)
    {
        float a = ((float)i / segments) * IM_PI * 2.0f + rot;
        float a2 = ((float)(i + 1) / segments) * IM_PI * 2.0f + rot;

        float x1 = center.x + cos(a) * radius;
        float y1 = center.y + sin(a) * radius;

        float x2 = center.x + cos(a2) * radius;
        float y2 = center.y + sin(a2) * radius;

        // rainbow color (HSV)
        float hue = (float)i / segments;
        ImU32 col = ImColor::HSV(hue, 1.0f, (*v ? 1.0f : 0.3f)); 

        dl->AddLine(ImVec2(x1, y1), ImVec2(x2, y2), col, 3.0f);
    }

    // glowing inner dot
    if (*v)
        dl->AddCircleFilled(center, radius * 0.55f, IM_COL32(255,255,255,230), 32);
    else
        dl->AddCircleFilled(center, radius * 0.55f, IM_COL32(80,80,80,200), 32);
